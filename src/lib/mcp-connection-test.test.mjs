    ...
    return Response.json({
      ...
      result:
        request.method === "initialize"
          ? { serverInfo: { name: "open-connect" }, protocolVersion: "2025-06-18" }
          : { tools: [{ name: "search" }] },
    });
  });
...
test("rejects invalid/revoked token and never displays response body", async () => {
  await assert.rejects(
    testMcpConnection("oc_live_test", async () => new Response("sensitive", { status: 401 })),
    /invalid or revoked/,
  );
});
...
test("rejects unsuccessful JSON-RPC responses", async () => {
  await assert.rejects(
    testMcpConnection("oc_live_test", async () =>
      Response.json({ jsonrpc: "2.0", id: 1, error: { message: "sensitive" } }),
    ),
...
test("rejects malformed discovery instead of reporting success", async () => {
  await assert.rejects(
    testMcpConnection("oc_live_test", async (_, options) => {
