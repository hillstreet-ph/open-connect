...
export function McpConnectionCard({ freshKey }: { freshKey: string | null }) {
  ...
  async function test() {
    ...
    try {
      ...
      setResult(
        `Connection verified: initialization succeeded and ${checked.toolCount} tools discovered. ChatGPT installation is a separate step.`,
      );
    } catch (error) {
      setResult(
        error instanceof Error && !["TypeError", "TimeoutError", "AbortError"].includes(error.name)
          ? error.message
          : "Could not reach MCP. Check your connection and try again.",
      );
    } finally {
      setBusy(false);
      setToken("");
    ...
    <Card className="shadow-panel">
      ...
      <CardContent className="space-y-4 text-sm">
        ...
        <ol className="list-decimal space-y-1 pl-5">
          <li>
            In ChatGPT Settings, open Security and login and enable Developer mode if available.
          </li>
          <li>Open Plugins, select the plus button, and add the MCP server URL above.</li>
          <li>
            Complete authentication, review the discovered tools, and add the connection to a new
            conversation.
          </li>
        </ol>
        <a
        ...
        {freshKey ? (
          <p>The test uses your newly created key.</p>
        ) : (
        ...
        <p className="text-xs text-muted-foreground">
          Tests initialization and tool discovery only. The key is sent to this site's MCP endpoint
          and is not saved by this test.
        </p>
